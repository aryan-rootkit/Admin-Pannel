...
function pctLine(label: string, pct: number | null, invert: boolean) {
  if (pct == null || !Number.isFinite(pct)) {
    return <p className="mt-2 text-[11px] text-purity-muted">Insufficient history</p>;
  }
  const up = pct >= 0;
  ...
  return (
    <p
      className={`mt-2 flex flex-wrap items-center gap-1 text-[11px] font-medium tabular-nums ${
...
export function DashboardBurnIntelligence({ burn }: Props) {
  ...
  return (
    <section aria-label="Burn and runway intelligence" className="space-y-4">
      <div>
        <h2 className={sectionLabel}>Burn &amp; runway intelligence</h2>
        <p className="mt-1 text-xs leading-snug text-purity-muted">
          Heuristic view: last-month payout outflow plus a twelfth of recorded subscriptions &amp; other
          expenses. Runway approximates pending revenue divided by that monthly burn proxy.
        </p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <article className={`${glassCard} ${cardPadding}`}>
          ...
          <p className="mt-2 text-[11px] leading-snug text-purity-muted">Payout month + opex slice</p>
        </article>
        ...
        <article className={`${glassCard} ${cardPadding}`}>
          ...
          <p className="mt-2 text-[11px] leading-snug text-purity-muted">Last month payout volume only</p>
        </article>

        <article className={`${glassCard} ${cardPadding}`}>
          <p className={sectionLabel}>Runway (pending ÷ burn)</p>
          <p className="mt-3 text-xl font-bold tabular-nums tracking-tight text-slate-900">{runway}</p>
          <p className="mt-2 text-[11px] leading-snug text-purity-muted">
        ...
        <article className={`${glassCard} ${cardPadding}`}>
          ...
          <p className="mt-2 text-[11px] leading-snug text-purity-muted">Composite burn vs prior month</p>
        </article>
      </div>
    </section>
  );
