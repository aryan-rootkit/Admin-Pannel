...
export const UltraQualityKanbanBoard: FC = () => {
  ...
  const renameColumn = (colId: string) => {
    const name = typeof window !== 'undefined' ? window.prompt('New column title') : null;
    if (!name) return;
    setColumns((cols) => cols.map((c) => (c.id === colId ? { ...c, title: name } : c)));
  };

  const addTask = (colId: string) => {
    const content = typeof window !== 'undefined' ? window.prompt('Task description') : null;
    if (!content) return;
    const id = Date.now().toString();
    setColumns((cols) =>
      cols.map((c) => (c.id === colId ? { ...c, tasks: [...c.tasks, { id, content }] } : c))
    );
  };
  ...
  const [draggingTask, setDraggingTask] = useState<{ colId: string; taskId: string } | null>(null);

  const onDragStart = (e: DragEvent<HTMLDivElement>, colId: string, taskId: string) => {
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center space-x-1">
              <button onClick={() => renameColumn(col.id)} className="p-1 hover:text-indigo-600">
                <Edit2 className="h-4 w-4" />
              </button>
              <button onClick={() => removeColumn(col.id)} className="p-1 hover:text-red-500">
          ...
          <div className="space-y-2 flex-1 overflow-y-auto">
            {col.tasks.map((task) => (
              ...
              >
                <span className="text-sm text-gray-800 dark:text-gray-100">{task.content}</span>
                <button onClick={() => removeTask(col.id, task.id)} className="p-1 hover:text-red-500">
                  <Trash2 className="h-4 w-4" />
                </button>
              </div>
            ))}
...
export default UltraQualityKanbanBoard;
